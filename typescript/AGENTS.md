...
│       ├── src/proxy.ts   reinfer() → { generateText, generateObject }
...
- **Zero-deps core**: `reinfer` has zero runtime dependencies.
- **Source-level dev**: `package.json` points at `./src/index.ts` during development. `publishConfig` swaps to `./dist/` for npm publishing.
- **Workspace protocol**: Packages reference each other via `"reinfer": "workspace:*"`.
